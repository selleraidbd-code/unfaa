...
export const AddCategoryDialog = ({
  ...
  const handleSaveAndClose = async () => {
    await updateShopThemeCategory({
      ...
      .catch((error) => {
        toast.error(
          error.data.message || "Something went wrong while updating categories"
        );
      });
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="lg:max-w-3xl">
            ...
            {categories.map((category: Category) => {
              ...
              return (
                <CategoryCard
                  ...
                  onSelect={() => handleSelectAndDeselect(category.id)}
                />
              );
            })}
          </DialogContainer>
        ...
        <DialogFooter className="grid grid-cols-2 gap-5">
          ...
          <CustomButton onClick={handleSaveAndClose} isLoading={isUpdating}>
            Save & Close
          </CustomButton>
        </DialogFooter>
      </DialogContent>
