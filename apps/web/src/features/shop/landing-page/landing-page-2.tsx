...
import { ensureCountryCode, toBengaliNumber } from "@/features/shop/landing-page/lib";
...
export const LandingPage02 = ({ landingPage, domain }: Props) => {
                            ...
                            <div className={cn("grid gap-8", aboutImageHave ? "md:grid-cols-2" : "")}>
                                <ul className="space-y-6">
                                    {aboutSection.sectionList?.map((service, index) => (
                                        <li key={service.id || index} className="flex items-start gap-4">
                                            <span className="center size-6 rounded bg-[var(--theme-primary)]">✔</span>
                                            <div>
                                                <p className="mb-2 text-lg leading-none font-bold">{service.title}</p>
                                ...
                                {aboutImageHave &&
                                    (() => {
                                        ...
                                        const isYoutubeUrl = !isEmbedHtml && /youtube\.com|youtu\.be/i.test(mediaUrl);
                                        const embedSrc =
