...
export const OrderDetailsModal = ({
    ...
    const handleCustomerInfoUpdate = (updatedOrder?: Order) => {
        // Update order data with the new customer information
        if (updatedOrder) {
            setOrderData((prev) => ({
                ...prev,
            ...
            // Fallback to original order if no updated data provided
            setOrderData(order);
        }
    };
                    ...
                    <DialogTitle className="flex justify-between pr-5">
                        <div className="space-y-1">
                            <p className="flex items-center gap-4">
                                Order Details{" "}
                                <CustomTextCopy text={orderData?.orderNumber.toString() || ""} copy={true} />
                            </p>
                            <span className="text-muted-foreground text-xs">
                                Last updated:{" "}
                                <time dateTime={orderData?.updatedAt || ""}>
                                    {formatDate(orderData?.updatedAt || "") || "N/A"}
                                </time>
                            </span>
                        </div>
                    </DialogTitle>
